...
- 必ず実装計画書に記載された実装セットごとに作業を行いなさい。まとめて作業は絶対にしてはいけません。クオリティが下がる原因になります。
...
- 実装結果レポートテンプレートは({{path("docs/implementation_report_template.md")}})から取得しなさい。
...
## 手順
0. noaqh-devコマンドが利用可能かチェックし、利用可能でない場合は処理を中止し、ユーザーに報告しなさい。
...
4. レビューガイドラインを取得し、レビューガイドラインを完全に理解しなさい。レビューガイドラインには例示が多く含まれています。この例示は非常に重要なので尊重されていることを確認しなさい。
5. 実装結果レポートテンプレートを取得し、テンプレートの構造を理解しなさい。
6. `spec/{feature_number}-{feature_name}/implementation_report.md`を作成し、テンプレートに従って初期状態を作成しなさい。
7. 実装計画書に記載された実装セットごとに以下の手順を実行しなさい。すべての実装セットに対して順番に作業を行い、まとめて作業は絶対にしてはいけません。
   7-1. 実装セットで行うべき内容を理解しなさい。実装計画書に記載されている実装セットの説明、実装内容、手順を確認しなさい。
   7-2. 実装セットの手順に従って、TDDのRed-Green-Refactorサイクルを意識しながら実装を行いなさい。
   ...
   7-3. 実装したコードがコードスタイルドキュメントに沿っているかを確認しなさい。違反があれば修正しなさい。
   7-4. 実装したコードがアーキテクチャドキュメントに沿っているかを確認しなさい。違反があれば修正しなさい。
   7-5. 実装したコードがレビューガイドラインに沿っているかを確認しなさい。違反があれば修正しなさい。
   7-6. テストを実行し、すべてのテストが通ることを確認しなさい。テストが失敗する場合は、原因を特定し修正しなさい。
   7-7. 実装計画書の実装セットに対応するチェックリスト項目を完了し、`- []`を`- [x]`に変更しなさい。
   7-8. 実装結果レポートを更新し、実装セットで実装した内容、動作確認の結果、発見した課題などを記載しなさい。
   7-9. 実装セットでの作業が完了したら、ユーザーに進捗を報告しなさい。必要に応じて、実装計画書に記載されていない技術的な選択が必要な場合は、ユーザーに確認を求めなさい。
8. すべての実装セットの作業が完了したら、実装結果レポートを完成させ、以下の観点で最終確認を行いなさい。
   8-1. 実装計画書に記載されたすべての実装セットが完了しているかを確認しなさい。
   8-2. 実装計画書通りに実装されているかを確認しなさい。
   8-3. コードスタイルドキュメントに沿っているかを確認しなさい。
   8-4. アーキテクチャドキュメントに沿っているかを確認しなさい。
   8-5. レビューガイドラインに沿っているかを確認しなさい。
   8-6. すべてのテストが通っているかを確認しなさい。
   8-7. Lintが通っているかを確認しなさい。
   8-8. 型エラーがないかを確認しなさい。
   8-9. ビルドが通っているかを確認しなさい。
   8-10. 実装結果レポートに実装内容の詳細、動作確認の結果、残タスク、所感・課題・次のアクションが適切に記載されているかを確認しなさい。
9. 最終確認で問題があれば修正しなさい。
10. `noaqh-dev review`コマンドを実行し、実装結果をレビューしなさい。対応が必要なものがあれば対応しなさい。
11. もしあなたがCursorの場合は実装結果レポートをユーザーに報告しなさい。
12. もしあなたがCursor以外(codex, Claude Code, Gemini, etc.)の場合はUI確認手順を実施しなさい。必ずすべてのUI確認手順を実施し、実装レポートに結果を記載しなさい。事実上実装されているから問題ないと判断は絶対にせず、実装レポートに結果を記載しなさい。
