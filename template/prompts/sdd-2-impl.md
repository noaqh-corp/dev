...
9. 最終確認で問題があれば修正しなさい。
10. もしあなたがCursorの場合は実装結果レポートをユーザーに報告しなさい。
11. もしあなたがCursor以外(codex, Claude Code, Gemini, etc.)の場合はUI確認手順を実施しなさい。必ずすべてのUI確認手順を実施し、実装レポートに結果を記載しなさい。事実上実装されているから問題ないと判断は絶対にせず、実装レポートに結果を記載しなさい。
