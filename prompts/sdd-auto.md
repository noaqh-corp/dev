...
#### ブラウザ確認プロンプト
```
/n-sdd-3-browser-check {実装結果レポートの相対パス}
```
...
2. 実装が完了したら実装フェーズは完了です。ブラウザ確認フェーズに移行しなさい。


## ブラウザ確認フェーズ
### ルール
- ブラウザ確認はClaudeCode、Codexでのみ実行可能です。それ以外の場合はユーザーに報告し、作業を中止しなさい。
- 問題が発生した場合は修正を指示し、解決するまでブラウザ確認プロンプトを実行しなさい。

### ブラウザ確認フェーズ完了判断基準
- すべての動作確認が正常に完了している。
- ブラウザ確認結果レポートが作成されている。

### 手順
1. ブラウザ確認プロンプトに実装結果レポートの相対パスを入力し、ブラウザ確認を指示しなさい。
2. 問題があれば修正が完了するまでブラウザ確認プロンプトを継続して実行しなさい。
3. すべての動作確認が正常に完了したらブラウザ確認フェーズは完了です。ユーザーへ完了報告を行いなさい。
