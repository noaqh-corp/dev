...
Issue: {対応するLinear IssueのURL または "存在しません"}
...
{
  要対応: UI確認手順を定義してください。
  ここでは実装した内容をすべて確認する手順を明記しなさい。テストだけでは確認できない項目を確認し、確実に動作させるための手順です。
}
 概要: {どのような変更があったため、どのようなUI確認を行うべきかを3行程度で記載してください。 ex: Todoに期限日を追加したため、期限日が正しく動作することを確認する。}

 ### 確認セット [確認セット名]
 {
  要対応: 確認セットを定義してください。上から順に実行すればタスクが完了するようにする。
  ログイン手順がある場合は必ずどのアカウントでどのようにログインするかを明記してください。
  また、 スクリーンショットをどこで撮影するかも重要です。スクリーンショットを撮影し、ユーザーがあとから簡単に見直しできるようにしなさい。スクリーンショットを撮影する手順では手順先頭に📸を付けてください。
}
 {1行程度で概要を記載してください。}
  - 事前準備手順:
    1. [準備手順1 ex: `localhost:3000`にアクセスし、サーバーが起動しているかを確認する。もし、サーバーが起動していない場合は`bun run dev`を実行し、サーバーを起動する。]
    2. [準備手順2 ex: `localhost:3000/debug/login?email=test@example.com`でログインし、ログインが成功しているかを確認する。]
  - 確認手順:
    1. 📸 [確認手順1 ex: `localhost:3000/todo/new`にアクセスし、期限日が正しく動作することを確認する。]
    2. 📸 [確認手順2 ex: `localhost:3000/todo/new`で`期限日が過去の日付`を入力した場合はエラーが表示されることを確認する。]
    3. 📸 [確認手順3 ex: `localhost:3000/todo/new`で`期限日が未来の日付`を入力した場合はエラーが表示されないことを確認する。]
